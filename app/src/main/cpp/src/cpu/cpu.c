 ...
 */
static inline void cpuApplyImeDelayAfterInstruction(CpuContext* ctx)
{
  // EI enables IME only after one subsequent instruction completes.
  if (ctx->imeDelay)
  {
    ctx->ime      = true;
    ctx->imeDelay = false;
...
void cpuTick(void)
{
  ...
  else if (ctx->mCycle == M1)
  {
    ...
    if (status == EXEC_STATUS_DONE_IMMEDIATE)
    {
      ctx->mCycle = M1;
      ctx->isCB   = false;
      cpuApplyImeDelayAfterInstruction(ctx);
    }
    else
    {
      ctx->mCycle = M2;
