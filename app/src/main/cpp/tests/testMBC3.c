...
static TestResult test_mbc3_rom_bank_switch_basic(void)
{
  u32 romSize = 0;
  u8* rom = makeTestRom(16, CART_TYPE_MBC3, 0x03 /* 256KiB */, 0x00, &romSize);
...
static TestResult test_mbc3_ram_enable_and_write_bank0(void)
{
  u32 romSize = 0;
  u8* rom = makeTestRom(16, CART_TYPE_MBC3, 0x03 /* 256KiB */, 0x03 /* 32KiB */, &romSize);

  EXPECT_TO_BE_TRUE(cartridgeInit(&TEST_IO, rom, romSize));

  cartridgeWrite(0x4000, 0x00); /* RAM bank 0 */
  EXPECT_TO_BE((u8)0xFF, cartridgeRead(0xA000));

  cartridgeWrite(0x0000, 0x0A);
  cartridgeWrite(0xA000, 0x66);
  EXPECT_TO_BE((u8)0x66, cartridgeRead(0xA000));

  free(rom);
  return TEST_RESULT_PASSED;
}

static TestResult test_mbc3_ram_banking_distinguishes_banks(void)
{
  u32 romSize = 0;
  u8* rom = makeTestRom(16, CART_TYPE_MBC3, 0x03 /* 256KiB */, 0x03 /* 32KiB */, &romSize);
  ...
  cartridgeWrite(0xA000, 0x10);

  cartridgeWrite(0x4000, 0x01);
  cartridgeWrite(0xA000, 0x11);

  cartridgeWrite(0x4000, 0x00);
  EXPECT_TO_BE((u8)0x10, cartridgeRead(0xA000));

  cartridgeWrite(0x4000, 0x01);
  EXPECT_TO_BE((u8)0x11, cartridgeRead(0xA000));
...
int main(void)
{
  registerTest(test_mbc3_rom_bank_switch_basic,           "MBC3: ROM bank switching (0->1 rule)");
  registerTest(test_mbc3_ram_enable_and_write_bank0,      "MBC3: RAM enable + RAM R/W (bank0)");
  registerTest(test_mbc3_ram_banking_distinguishes_banks, "MBC3: RAM banking keeps banks distinct");
  runTests();
  return 0;
}
