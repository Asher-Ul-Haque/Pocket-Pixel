...
const char* getInstructionName(InstructionType TYPE) { return instructionLookup[TYPE].c_str(); }

static std::string regLookup[] = 
  {
    "<NONE>",
    "A",
    "F",
...
void instructionToStr(CPUcontext* CTX, char* STR) 
{
    ...
    sprintf(STR, "%s ", getInstructionName(inst->type));
    ...
    switch(inst->mode) 
    {
        ...
        case AM_R_D16:
        case AM_R_A16:
            sprintf(STR, "%s %s,$%04X", getInstructionName(inst->type), 
                regLookup[inst->reg1].c_str(), CTX->readData);
            return;

        case AM_R:
            sprintf(STR, "%s %s", getInstructionName(inst->type), 
                regLookup[inst->reg1].c_str());
            return;

        case AM_R_R: 
            sprintf(STR, "%s %s,%s", getInstructionName(inst->type), 
                regLookup[inst->reg1].c_str(), regLookup[inst->reg2].c_str());
            return;
        ...
        case AM_R_D8:
        case AM_R_A8:
            sprintf(STR, "%s %s,$%02X", getInstructionName(inst->type), 
                regLookup[inst->reg1].c_str(), CTX->readData & 0xFF);
            return;
        ...
        case AM_A8_R:
            sprintf(STR, "%s $%02X,%s", getInstructionName(inst->type), 
                busRead(CTX->regs.programCounter - 1), regLookup[inst->reg2].c_str());

            return;

        case AM_HL_SPR:
            sprintf(STR, "%s (%s),SP+%d", getInstructionName(inst->type), 
                regLookup[inst->reg1].c_str(), CTX->readData & 0xFF);
            return;

        case AM_D8:
            sprintf(STR, "%s $%02X", getInstructionName(inst->type), 
                CTX->readData & 0xFF);
            return;

        case AM_D16:
            sprintf(STR, "%s $%04X", getInstructionName(inst->type), 
                CTX->readData);
            return;
        ...
        case AM_A16_R:
            sprintf(STR, "%s ($%04X),%s", getInstructionName(inst->type), 
                CTX->readData, regLookup[inst->reg2].c_str());
            return;

        default:
            FORGE_LOG_FATAL("INVALID AM: %d", inst->mode);
            FORGE_ASSERT(false);
    }
}
