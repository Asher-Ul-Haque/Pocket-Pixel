...
) : BoxArtFetcher
{
  ...
  private fun findClosest(
    ...
    return if (THRESHOLD > 0.0) {
      best?.takeIf { it.second >= THRESHOLD }?.first
    } else {
      best?.first
    }
  }
  ...
  override fun fetchBoxArt(GAME_NAME: String): Flow<String?> = flow {
    ...
    val cachedBoxArtFile = cacheJson[GAME_NAME]
    if (!cachedBoxArtFile.isNullOrBlank() && cachedBoxArtFile.endsWith(".png")) {
      ForgeLogger.info("Found it in the cache: $cachedBoxArtFile")
      emit(BASE_URL + urlEncode(cachedBoxArtFile))
      return@flow
    } else {
      ForgeLogger.info("Cache entry for [$GAME_NAME] was blank or invalid. Proceeding to fetch.")
    }

    val possibleExactName = stripExtension(GAME_NAME) + ".png"
    val exactUrl = BASE_URL + urlEncode(possibleExactName)

    ForgeLogger.info("Checking for exact file: $possibleExactName\nURL: $exactUrl")

    when (val headResult = NETWORK.head(exactUrl)) {
      is NetworkResult.Success -> {
        if (headResult.data) {
          ForgeLogger.info("Found exact match on the internet")
    ...
    ForgeLogger.info("Exact match failed. Starting fuzzy search for: $GAME_NAME")
    ...
    if (!closest.isNullOrBlank()) {
      ...
      if (closestFileName != null) {
        ForgeLogger.info("Found closest fuzzy match: $closestFileName")
      ...
      ForgeLogger.info("Fuzzy search yielded blank result. Not saving empty value to cache.")
    }

    ForgeLogger.info("Found no match for $GAME_NAME")
    emit(null)
  }
